...
if (!source.includes('const desktopRoomAccent =')) {
  const anchor = '  const desktopRoom = rooms.find((room) => room.area.area_id === desktopRoomId) ?? rooms[0] ?? null;';
  ...
  if (!source.includes(anchor)) throw new Error('desktopRoom anchor not found');
  source = source.replace(anchor, replacement);
}

const oldDesktopBlock = `          <div className="desktop-room-stage-v26">\n            {desktopRoom && (\n              <RoomCard\n                key={\`desktop-${'${'}desktopRoom.area.area_id}\`}\n                hass={hass}\n                room={desktopRoom}\n                language={language}\n                onClimate={(entityId) => setRoomClimateEntityId(entityId)}\n              />\n            )}\n          </div>`;

const newDesktopBlock = `          <div className="desktop-room-stage-v26">\n            {desktopRoom && (\n              <DesktopRoomPanel\n                key={\`desktop-${'${'}desktopRoom.area.area_id}\`}\n                hass={hass}\n                room={desktopRoom}\n                language={language}\n                accent={desktopRoomAccent}\n                onClimate={(entityId) => setRoomClimateEntityId(entityId)}\n              />\n            )}\n          </div>`;

if (!source.includes('<DesktopRoomPanel')) {
  if (!source.includes(oldDesktopBlock)) throw new Error('desktop RoomCard block not found');
  source = source.replace(oldDesktopBlock, newDesktopBlock);
}

if (!source.includes('function DesktopRoomPanel(')) {
  const insertAnchor = '\nfunction RoomCard(';
  if (!source.includes(insertAnchor)) throw new Error('RoomCard insertion anchor not found');
  const component = `
    ...
    <article className="desktop-room-panel-v32" style={{ "--desktop-room-accent": accent } as CSSProperties}>
      <header className="desktop-room-header-v32">
        <div className="desktop-room-title-v32">
          ...
          <div><strong>{room.area.name}</strong><small>{activeCount > 0 ? `${'${'}activeCount} ${'${'}language === "it" ? "attivi" : "active"}` : (language === "it" ? "Tutto spento" : "All off")}</small></div>
        </div>
        <button
          type="button"
...
`;
  source = source.replace(insertAnchor, `\n${component}\nfunction RoomCard(`);
}
