      ...
      {page === "calendar" ? (
        <>
          <LargeCalendarWorkspace
            ...
            onSelectedEntity={setSelectedEntity}
            onTurnOffScope={turnOffScope}
          />
          <section className="dashboard-grid phone-calendar-layout-v60">
...
function LargeCalendarWorkspace({
  hass,
  language,
  now,
  events,
  ...
  onSelectedEntity: (entityId: string | null) => void;
  onTurnOffScope: () => void | Promise<void>;
}) {
  return (
    <section className="v60-calendar-workspace" aria-label={t("calendar", language)}>
      <LargeMonthV60
        month={visibleMonth}
        today={now}
        events={events}
      ...
      <aside className="v60-family-rail" aria-label={language === "it" ? "Pannello famiglia" : "Family panel"}>
        <LargeAgendaV60 now={now} events={events} language={language} />
...
function LargeMonthV60({
  ...
  return (
    <section className="v60-month">
      <header className="v60-month-head">
        <div>
          <span className="v60-eyebrow">{t("calendar", language)}</span>
          <h1>{month.toLocaleDateString(localeFor(language), { month: "long", year: "numeric" })}</h1>
        </div>
        <div className="v60-month-nav">
          <button type="button" onClick={onPrevious} aria-label={t("previousMonth", language)}><ChevronLeftIcon /></button>
          <button type="button" className="v60-today" onClick={onToday}>{t("todayButton", language)}</button>
          <button type="button" onClick={onNext} aria-label={t("nextMonth", language)}><ChevronRightIcon /></button>
        </div>
      </header>

      <div className="v60-month-grid">
        {weekdays.map((day) => <div className="v60-weekday" key={day}>{day}</div>)}
        {cells.map(({ date, outside }) => {
          const key = dateKey(date);
          const dayEvents = events.filter((event) => isDateInsideEvent(key, event));
          const isToday = key === dateKey(today);
          return (
            <div className={`v60-day ${outside ? "outside" : ""} ${isToday ? "today" : ""}`} key={key}>
              <div className="v60-day-number">
                <span>{date.getDate()}</span>
                {isToday && <i />}
              </div>
              <div className="v60-day-events">
                {dayEvents.map((event) => {
                  const starts = key === event.startDate;
                  const ends = key === (event.endDate ?? event.startDate);
                  return (
                    <div
                      className={`v60-event ${event.tone ?? "mint"} ${!starts ? "continues-before" : ""} ${!ends ? "continues-after" : ""}`}
                      key={`${event.startDate}-${event.title}`}
                    >
                      {starts && event.time && <span>{event.time}</span>}
                      <strong>{starts ? event.title : ""}</strong>
                    </div>
                  );
                })}
              </div>
...
function LargeAgendaV60({ now, events, language }: { now: Date; events: DemoEvent[]; language: Language }) {
  ...
  return (
    <section className="v60-rail-section v60-agenda">
      <header className="v60-section-head">
        <div><span className="v60-eyebrow">{t("agenda", language)}</span><h2>{t("upcoming", language)}</h2></div>
        <b>{groups.reduce((sum, group) => sum + group.events.length, 0)}</b>
      </header>
      <div className="v60-agenda-list">
        {groups.map(({ date, key, events: dayEvents }, index) => (
          <div className="v60-agenda-day" key={key}>
            <div className="v60-agenda-date">
              <strong>{index === 0 ? t("today", language) : index === 1 ? t("tomorrow", language) : date.toLocaleDateString(localeFor(language), { weekday: "short" })}</strong>
              <span>{date.toLocaleDateString(localeFor(language), { day: "2-digit", month: "short" })}</span>
            </div>
            <div className="v60-agenda-events">
              {dayEvents.length === 0
                ? <small>{t("noEvents", language)}</small>
                : dayEvents.map((event) => (
                  <div className="v60-agenda-event" key={`${event.startDate}-${event.title}`}>
                    <span>{key === event.startDate ? (event.time ?? "•") : "↳"}</span>
...
function LargeTasksV60({
  ...
  return (
    <section className="v60-rail-section v60-tasks">
      <header className="v60-section-head">
        <div><span className="v60-eyebrow">{t("lists", language)}</span><h2>{mode === "todo" ? t("todo", language) : t("shopping", language)}</h2></div>
        <button type="button" className={addingTask ? "active" : ""} onClick={onToggleAdding} aria-label={addingTask ? t("close", language) : t("add", language)}>
          {addingTask ? <CloseIcon /> : <PlusIcon />}
        </button>
      </header>

      {addingTask && (
        <form className="v60-task-composer" onSubmit={onAddTask} noValidate>
          <input
            autoFocus
            value={taskDraft}
            onChange={(event) => onTaskDraft(event.target.value)}
            onKeyDown={(event) => { if (event.key === "Escape") onCancelTask(); }}
            placeholder={language === "it" ? "Nuovo elemento" : "New item"}
            aria-label={t("add", language)}
          />
      ...
      <div className="v60-task-list">
        {currentTasks.map((item) => (
          <div className={`v60-task-row ${item.done ? "done" : ""}`} key={item.id}>
            <label>
              <input type="checkbox" checked={item.done} onChange={() => onUpdateTask(item.id)} />
              <span className="v60-task-check" aria-hidden="true"><CheckIcon /></span>
              <span>{item.label}</span>
            </label>
            <button type="button" onClick={() => onDeleteTask(item.id)} aria-label={t("delete", language)} title={t("delete", language)}><TrashIcon /></button>
          </div>
        ))}
      </div>

      <div className="v60-task-tabs" role="tablist" aria-label={t("lists", language)}>
        <button type="button" role="tab" aria-selected={mode === "todo"} className={mode === "todo" ? "active" : ""} onClick={() => onMode("todo")}>{t("todo", language)}</button>
        <button type="button" role="tab" aria-selected={mode === "shopping"} className={mode === "shopping" ? "active" : ""} onClick={() => onMode("shopping")}>{t("shopping", language)}</button>
      </div>
    </section>
  );
}

function LargeHomePeekV60({
  hass,
  language,
  areas,
  room,
  ...
  const roomName = room === "__favorites"
    ? t("favorites", language)
    : areas.find((area) => area.area_id === room)?.name ?? t("home", language);

  const visibleEntities = roomEntities.filter((entityId) => {
    const domain = entityId.split(".")[0];
    return !isPassiveDashboardEntity(hass, entityId) || ["sensor", "binary_sensor"].includes(domain);
  });

  return (
    <section className="v60-rail-section v60-home-peek">
      <header className="v60-section-head">
        <div><span className="v60-eyebrow">{t("smartHome", language)}</span><h2>{roomName}</h2></div>
        <button type="button" className="v60-power-all" onClick={() => void onTurnOffScope()} aria-label={t("turnOffAll", language)} title={t("turnOffAll", language)}><PowerIcon /></button>
      </header>

      <div className="v60-room-tabs" role="tablist" aria-label={t("room", language)}>
        <button type="button" role="tab" aria-selected={room === "__favorites"} className={room === "__favorites" ? "active" : ""} onClick={() => onRoom("__favorites")}><StarIcon /><span>{t("favorites", language)}</span></button>
        {areas.map((area) => (
          <button type="button" role="tab" aria-selected={room === area.area_id} className={room === area.area_id ? "active" : ""} key={area.area_id} onClick={() => onRoom(area.area_id)}><i /><span>{area.name}</span></button>
        ))}
      </div>

      <div className="v60-home-peek-grid">
        {visibleEntities.length === 0 && <div className="v60-empty">{t("noDevices", language)}</div>}
        {visibleEntities.map((entityId) => {
          ...
          return (
            <article
              className={`v60-peek-device domain-${domain} ${passive ? "passive" : ""} ${active ? "active" : ""} ${selectedEntity === entityId ? "selected" : ""}`}
              style={accessoryStyle(hass, entityId)}
              key={entityId}
            >
              {passive ? (
                <div className="v60-peek-device-main" aria-label={`${displayName(hass, entityId)}: ${status}`}>
                  <span>{iconForEntity(hass, entityId)}</span><div><strong>{displayName(hass, entityId)}</strong><small>{status}</small></div>
                </div>
              ) : (
                <button type="button" className="v60-peek-device-main" onClick={() => void activateEntity(hass, entityId)}>
                  <span>{iconForEntity(hass, entityId)}</span><div><strong>{displayName(hass, entityId)}</strong><small>{status}</small></div>
                </button>
              )}
              <button type="button" className={`v60-peek-favorite ${favorites.includes(entityId) ? "selected" : ""}`} aria-label={t("favorites", language)} onClick={() => void onToggleFavorite(entityId)}><StarIcon /></button>
              {configurable && <button type="button" className="v60-peek-configure" aria-label={t("controls", language)} onClick={() => onSelectedEntity(selectedEntity === entityId ? null : entityId)}><SlidersIcon /></button>}
            </article>
          );
        })}
      </div>
      ...
      {selectedEntity && hass.states[selectedEntity] && (
        <DeviceControls hass={hass} entityId={selectedEntity} language={language} onClose={() => onSelectedEntity(null)} />
      )}
    </section>
  );
}
